...
def page_valuation(artifacts):
    ...
    with col_results:
        if predict_btn:
            ...
            for col, (factor, imp) in zip(factor_cols, result['factors']):
                ...
                with col:
                    ...
                    <div style="
                        ...
                        border: 1px solid rgba(255,255,255,0.05);
                    ">
                        <div style="font-size: 1.4rem; font-weight: 800; color: #00c853;">
                            {imp*100:.1f}%
                        </div>
                        <div style="font-size: 0.75rem; color: #8899a6; margin-top: 6px;">
                            {clean}
                        </div>
                    </div>
                    """)
            ...
            st.html("<br>")
            st.html("#### Tier Probability Distribution")
            probs = result['probs']
            ...
            # Comparative context
            st.markdown("<br>")
            st.html("#### Attribute Context vs. Dataset")
            ...
            <div style="
                ...
                padding: 70px 40px;
                text-align: center;
                border: 2px dashed rgba(0,200,83,0.15);
                margin-top: 20px;
            ">
...
def page_similar(artifacts):
    ...
    with tab1:
        ...
        if st.button("Find Comparables", key="sim_existing", type="primary"):
            ...
            st.html(f"#### Top 5 Comparables for {selected}")
...
def page_about(artifacts):
    ...
    # Performance metrics
    st.html("### Test-Set Performance Summary")
    m = artifacts['metrics']
    perf_data = {
    ...
    # Model comparison
    st.markdown("<br>")
    st.html("### Algorithm Family Comparison")
    comp_data = {
    ...
    st.markdown("<br>")
    st.html("### Feature Importance — Regression Ensemble")
    gbr = artifacts['models']['gbr_reg']
    names = artifacts['all_feature_names']
    imp = pd.DataFrame({
        'Feature': [n.replace('_', ' ').replace('pos ', '').title() for n in names],
...
def main():
    ...
    with st.sidebar:
        ...
        st.html("<hr style='border-color: rgba(255,255,255,0.08); margin: 30px 0;'>")
