...
# CSS injection - single line to avoid markdown code blocks
st.markdown("<style>" +
    "@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap');" +
    "html, body, [class*=\"css\"] { font-family: 'Inter', sans-serif !important; }" +
    ".stApp { background-color: #0a0e17; }" +
    "[data-testid=\"stSidebar\"] { background-color: #111827; border-right: 1px solid rgba(255,255,255,0.05); }" +
    "h1 { color: #ffffff; font-weight: 800; letter-spacing: -0.5px; }" +
    "h2 { color: #e0e0e0; font-weight: 700; border-left: 4px solid #00c853; padding-left: 16px; margin-top: 30px; }" +
    "h3 { color: #b0b0b0; font-weight: 600; }" +
    "p, li { color: #8899a6; }" +
    ".stButton>button { background: linear-gradient(90deg, #00c853 0%, #009624 100%); color: white; border: none; border-radius: 12px; padding: 14px 24px; font-size: 16px; font-weight: 700; letter-spacing: 0.5px; transition: all 0.3s ease; width: 100%; }" +
    ".stButton>button:hover { box-shadow: 0 0 30px rgba(0,200,83,0.4); transform: translateY(-2px); }" +
    ".stSlider > div > div > div { color: #00c853; }" +
    ".dataframe { background-color: #151b2b !important; color: #e0e0e0 !important; }" +
    "th { background-color: #1a2332 !important; color: #00c853 !important; font-weight: 600 !important; }" +
    ...
    ".stTabs [data-baseweb=\"tab\"] { background-color: #151b2b; border-radius: 8px 8px 0 0; color: #8899a6; font-weight: 600; border: none; padding: 10px 20px; }" +
    ".stTabs [aria-selected=\"true\"] { background-color: #1a2332 !important; color: #00c853 !important; border-bottom: 2px solid #00c853 !important; }" +
    ...
    "[data-testid=\"stMetricDelta\"] { color: #00c853; }" +
...
def generate_synthetic_data(n=2500):
    ...
    teams = ['Manchester City', 'Real Madrid', 'Barcelona', 'Bayern Munich', 'Paris SG',
    ...
    base = ((data['Overall_Rating'] ** 2.3) * 0.008 + (data['Future Potential'] ** 1.9) * 0.004 + data['Total_Stats Score'] * 0.0007)
    base *= (1 + age_factor * 2.5)
    pos_mult = {'ST': 1.45, 'CF': 1.35, 'CAM': 1.25, 'LW': 1.40, 'RW': 1.40,
        'CM': 1.05, 'CDM': 0.95, 'LM': 1.0, 'RM': 1.0,
        'CB': 0.90, 'LB': 0.85, 'RB': 0.85, 'LWB': 0.90, 'RWB': 0.90, 'GK': 0.80}
...
def build_pipeline(df):
    ...
    svc_clf = SVC(kernel='rbf', C=10.0, gamma='scale', probability=True, random_state=RANDOM_STATE)
    ...
    ensemble_reg = VotingRegressor([('gbr', gbr_reg), ('svr', svr_reg), ('knn', knn_reg)])
    ensemble_clf = VotingClassifier([('gbr', gbr_clf), ('svc', svc_clf), ('knn', knn_clf)], voting='soft')
    ...
    for col in pos_cols:
        if col not in pos_dummies_full.columns: pos_dummies_full[col] = 0
    ...
    artifacts = {
        'df': df, 'scaler': scaler, 'label_encoder': le,
        'country_map': country_map, 'team_map': team_map, 'global_mean': global_mean,
        'pos_cols': pos_cols, 'numeric_features': numeric_features,
        'engineered_cols': engineered_cols,
        'all_feature_names': engineered_cols + pos_cols,
        'models': {
            'regression': ensemble_reg, 'classification': ensemble_clf,
            'gbr_reg': gbr_reg, 'gbr_clf': gbr_clf
        },
        'metrics': {
...
def predict_player(player_dict, artifacts):
    ...
    imp_list = sorted([(names[i], float(importances[i])) for i in range(len(names))], key=lambda x: x[1], reverse=True)[:6]
    return {
        'value': value, 'tier': tier, 'confidence': confidence,
        'factors': imp_list,
        'probs': {artifacts['label_encoder'].inverse_transform([i])[0]: float(p) for i, p in enumerate(probs)}
    }
...
def find_similar(player_dict, artifacts, n=6):
    ...
    for idx in top_idx:
        row = df.iloc[idx]
        out.append({
            'name': row['Name'], 'similarity': float(sims[idx]),
            'value': float(row['Value Per M$']), 'overall': int(row['Overall_Rating']),
            'potential': int(row['Future Potential']), 'age': int(row['Age']),
            'position': str(row['Position']), 'tier': str(row['Performance_Tier']),
...
def dark_layout(fig, title=None, height=None):
    fig.update_layout(
        ...
        margin=dict(l=60, r=40, t=60 if title else 30, b=40), height=height
    )
    fig.update_xaxes(gridcolor='rgba(255,255,255,0.06)', zerolinecolor='rgba(255,255,255,0.1)', showline=True, linecolor='rgba(255,255,255,0.1)')
    fig.update_yaxes(gridcolor='rgba(255,255,255,0.06)', zerolinecolor='rgba(255,255,255,0.1)', showline=True, linecolor='rgba(255,255,255,0.1)')
    return fig
...
# ============================================================
def render_html(html_string, height=0):
    """Render HTML using components.v1.html for full-width, markdown-free rendering."""
    wrapped = f'<div style="width:100%; font-family: Inter, sans-serif;">{html_string}</div>'
...
def tier_badge_html(tier):
    ...
    return f'<span style="background: linear-gradient(90deg, {bg}, {bg}dd); color: {fg}; padding: 6px 16px; border-radius: 20px; font-weight: 800; font-size: 0.85rem; letter-spacing: 1px; display: inline-block; box-shadow: 0 2px 10px {bg}40;">{label}</span>'
...
def page_home(artifacts):
    render_html("""
    <div style="background: linear-gradient(135deg, #0f1724 0%, #1a2332 50%, #0d1f15 100%); border-radius: 24px; padding: 50px 40px; border: 1px solid rgba(0,200,83,0.12); box-shadow: 0 20px 60px rgba(0,0,0,0.5); margin-bottom: 40px; position: relative; overflow: hidden;">
        <div style="position: absolute; top: -50%; right: -10%; width: 600px; height: 600px; background: radial-gradient(circle, rgba(0,200,83,0.06) 0%, transparent 70%); pointer-events: none;"></div>
        <h1 style="font-size: 3.5rem; font-weight: 800; color: #ffffff; margin: 0; letter-spacing: -1px;">⚽ SCOUTAI <span style="color: #00c853;">PRO</span></h1>
        <p style="font-size: 1.25rem; color: #8899a6; margin-top: 16px; max-width: 650px; line-height: 1.6;">Production-grade football intelligence for player valuation, performance tiering, and scouting operations. Powered by ensemble machine learning on multi-dimensional player attributes.</p>
    </div>
    """, height=280)
    ...
    with c1:
        render_html(f'<div style="background: linear-gradient(145deg, #151b2b, #1a2332); border-radius: 16px; padding: 24px; border-left: 4px solid #00c853; box-shadow: 0 4px 20px rgba(0,0,0,0.3); margin-bottom: 16px;"><p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1.5px;">📈 R² Score</p><h2 style="color: #ffffff; font-size: 2.2rem; margin: 8px 0; font-weight: 800;">{m["r2"]:.3f}</h2><p style="color: #00c853; font-size: 0.85rem; margin: 0; font-weight: 500;">Regression accuracy</p></div>', height=140)
    with c2:
        render_html(f'<div style="background: linear-gradient(145deg, #151b2b, #1a2332); border-radius: 16px; padding: 24px; border-left: 4px solid #ff6b6b; box-shadow: 0 4px 20px rgba(0,0,0,0.3); margin-bottom: 16px;"><p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1.5px;">📉 MAE</p><h2 style="color: #ffffff; font-size: 2.2rem; margin: 8px 0; font-weight: 800;">€{m["mae"]:.2f}M</h2><p style="color: #ff6b6b; font-size: 0.85rem; margin: 0; font-weight: 500;">Mean absolute error</p></div>', height=140)
    with c3:
        render_html(f'<div style="background: linear-gradient(145deg, #151b2b, #1a2332); border-radius: 16px; padding: 24px; border-left: 4px solid #448aff; box-shadow: 0 4px 20px rgba(0,0,0,0.3); margin-bottom: 16px;"><p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1.5px;">🗄️ Dataset</p><h2 style="color: #ffffff; font-size: 2.2rem; margin: 8px 0; font-weight: 800;">{len(artifacts["df"]):,}</h2><p style="color: #448aff; font-size: 0.85rem; margin: 0; font-weight: 500;">Players analyzed</p></div>', height=140)
    with c4:
        render_html(f'<div style="background: linear-gradient(145deg, #151b2b, #1a2332); border-radius: 16px; padding: 24px; border-left: 4px solid #ffd700; box-shadow: 0 4px 20px rgba(0,0,0,0.3); margin-bottom: 16px;"><p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1.5px;">🔧 Dimensions</p><h2 style="color: #ffffff; font-size: 2.2rem; margin: 8px 0; font-weight: 800;">{len(artifacts["all_feature_names"])}</h2><p style="color: #ffd700; font-size: 0.85rem; margin: 0; font-weight: 500;">Model features</p></div>', height=140)
    ...
    with left:
        render_html("""
        <div style="background: #151b2b; border-radius: 16px; padding: 28px; border: 1px solid rgba(255,255,255,0.05); height: 100%;">
            <h3 style="color: #ffffff; margin-top: 0;">Platform Overview</h3>
            <p style="color: #b0b0b0; line-height: 1.8; font-size: 0.95rem;">ScoutAI Pro combines gradient boosting, kernel SVM, and instance-based KNN models into a unified ensemble architecture. The platform processes player demographics, performance ratings, and club context to generate robust market value estimates and tier classifications.</p>
            <div style="display: flex; gap: 12px; margin-top: 24px;">
                <div style="flex: 1; background: #1a2332; border-radius: 12px; padding: 16px; text-align: center;"><div style="font-size: 1.5rem; font-weight: 700; color: #00c853;">3</div><div style="font-size: 0.75rem; color: #8899a6; margin-top: 4px;">Model Families</div></div>
                <div style="flex: 1; background: #1a2332; border-radius: 12px; padding: 16px; text-align: center;"><div style="font-size: 1.5rem; font-weight: 700; color: #00c853;">2</div><div style="font-size: 0.75rem; color: #8899a6; margin-top: 4px;">Ensemble Types</div></div>
                <div style="flex: 1; background: #1a2332; border-radius: 12px; padding: 16px; text-align: center;"><div style="font-size: 1.5rem; font-weight: 700; color: #00c853;">5-Fold</div><div style="font-size: 0.75rem; color: #8899a6; margin-top: 4px;">Cross Validation</div></div>
            </div>
        </div>
        """, height=300)
    ...
    with right:
        df = artifacts['df']
        tier_counts = df['Performance_Tier'].value_counts().reindex(['Elite', 'Good', 'Average', 'Developing']).fillna(0)
        colors = ['#ffd700', '#00c853', '#448aff', '#78909c']
        fig = go.Figure(data=[go.Pie(
            labels=tier_counts.index, values=tier_counts.values, hole=0.65,
            marker_colors=colors, textinfo='label+percent', textfont_size=11,
        ...
        fig.update_layout(
            ...
            showlegend=False, height=360, template='plotly_dark',
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
            font_color='#e0e0e0', margin=dict(t=50, b=20)
        )
        fig.add_annotation(text=f"<b>{len(df):,}</b><br>PLAYERS", x=0.5, y=0.5, font_size=15, showarrow=False, font_color='#ffffff')
    ...
    with i1:
        ...
        render_html(f'<div style="background: #151b2b; border-radius: 12px; padding: 20px; border: 1px solid rgba(255,255,255,0.05);"><p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1px;">Highest Valuation</p><p style="color: #ffffff; font-weight: 700; font-size: 1.1rem; margin: 8px 0 0 0;">{top_val["Name"]}</p><p style="color: #00c853; font-weight: 700; margin: 4px 0 0 0;">€{top_val["Value Per M$"]:.2f}M</p></div>', height=120)
    with i2:
        avg_val = df['Value Per M$'].mean()
        render_html(f'<div style="background: #151b2b; border-radius: 12px; padding: 20px; border: 1px solid rgba(255,255,255,0.05);"><p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1px;">Market Average</p><p style="color: #ffffff; font-weight: 700; font-size: 1.1rem; margin: 8px 0 0 0;">€{avg_val:.2f}M</p><p style="color: #8899a6; font-size: 0.8rem; margin: 4px 0 0 0;">Across all positions</p></div>', height=120)
    with i3:
        elite_pct = (df['Performance_Tier'] == 'Elite').mean() * 100
        render_html(f'<div style="background: #151b2b; border-radius: 12px; padding: 20px; border: 1px solid rgba(255,255,255,0.05);"><p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1px;">Elite Talent Pool</p><p style="color: #ffffff; font-weight: 700; font-size: 1.1rem; margin: 8px 0 0 0;">{elite_pct:.1f}%</p><p style="color: #ffd700; font-size: 0.8rem; margin: 4px 0 0 0;">≥80 Overall Rating</p></div>', height=120)
...
def page_valuation(artifacts):
    st.markdown("## 💰 Player Valuation Engine")
    st.markdown("<p style='color: #8899a6; margin: 0; font-size: 1rem;'>Input a player attribute profile to generate an AI-powered market value estimate and performance tier classification.</p>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    with col_inputs:
        render_html('<div style="background: #151b2b; border-radius: 16px; padding: 24px; border: 1px solid rgba(255,255,255,0.05);"><h4 style="color: #00c853; margin-top: 0; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1.5px;">Attribute Profile</h4></div>', height=80)

        with st.container():
            c1, c2 = st.columns(2)
            with c1:
                age = st.slider("Age", 16, 40, 24, help="Current age in years")
                overall = st.slider("Overall Rating", 45, 95, 76, help="Current overall ability (EA Sports scale)")
            with c2:
                potential = st.slider("Future Potential", 50, 99, 81, help="Projected peak overall rating")
                total_stats = st.slider("Total Stats Score", 700, 2700, 1850, help="Aggregate of all technical, physical and mental attributes")
    ...
    with col_results:
        if predict_btn:
            player = {
                ...
                'Total_Stats Score': total_stats, 'Country': country, 'Team': team, 'Position': position
            }
            result = predict_player(player, artifacts)
            tier = result['tier']
            tier_color = {'Elite': '#ffd700', 'Good': '#00c853', 'Average': '#448aff', 'Developing': '#78909c'}.get(tier, '#78909c')

            render_html(f"""
            <div style="background: linear-gradient(145deg, #151b2b, #1a2332); border-radius: 20px; padding: 32px; border: 1px solid {tier_color}40; box-shadow: 0 8px 32px {tier_color}18; margin-bottom: 24px;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px;">
                    <div>
                        <p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1.5px;">Estimated Market Value</p>
                        <h1 style="color: #ffffff; font-size: 3.2rem; margin: 8px 0; font-weight: 800;">€{result['value']:.2f}M</h1>
                    </div>
                    <div style="text-align: right;">
                        <p style="color: #8899a6; font-size: 0.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1.5px;">Performance Tier</p>
                        <div style="margin-top: 10px;">{tier_badge_html(tier)}</div>
                    </div>
                </div>
                <div style="background: rgba(0,0,0,0.2); border-radius: 12px; padding: 16px; margin-bottom: 20px;">
                    ...
                    <div style="background: #0a0e17; border-radius: 6px; height: 10px; overflow: hidden;">
                        <div style="width: {result['confidence']*100}%; background: linear-gradient(90deg, {tier_color}, {tier_color}80); height: 100%; border-radius: 6px; transition: width 0.6s ease;"></div>
                    </div>
                </div>
            </div>
            """, height=280)
            ...
            for col, (factor, imp) in zip(factor_cols, result['factors']):
                ...
                with col:
                    render_html(f'<div style="background: #151b2b; border-radius: 12px; padding: 16px; text-align: center; border: 1px solid rgba(255,255,255,0.05);"><div style="font-size: 1.4rem; font-weight: 800; color: #00c853;">{imp*100:.1f}%</div><div style="font-size: 0.75rem; color: #8899a6; margin-top: 6px;">{clean}</div></div>', height=90)
            ...
            fig = go.Figure(go.Bar(
                x=prob_df['Tier'], y=prob_df['Probability'], marker_color=bar_colors,
            ...
            fig.add_trace(go.Bar(
                ...
                marker_color='#00c853', text=comp_data['Player'].round(0), textposition='outside'
            ))
            fig.add_trace(go.Bar(
                name='Dataset Average', x=comp_data['Attribute'], y=comp_data['Dataset Avg'],
                marker_color='rgba(255,255,255,0.15)', text=comp_data['Dataset Avg'].round(0), textposition='outside'
            ))
            ...
            <div style="background: #151b2b; border-radius: 20px; padding: 70px 40px; text-align: center; border: 2px dashed rgba(0,200,83,0.15); margin-top: 20px;">
                ...
                <p style="color: #8899a6; margin-top: 10px; max-width: 400px; margin-left: auto; margin-right: auto;">Configure the player attribute profile and click the button above to receive an AI-powered market analysis with tier classification.</p>
            </div>
            """, height=280)
...
def page_analytics(artifacts):
    ...
    st.markdown("<p style='color: #8899a6; margin: 0; font-size: 1rem;'>Interactive exploration of market trends, attribute correlations, and squad composition.</p>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    with st.expander("🔍 Analysis Filters", expanded=True):
        ...
        with f3:
            overall_range = st.slider("Overall Range", int(df['Overall_Rating'].min()), int(df['Overall_Rating'].max()),
    ...
    c1, c2 = st.columns(2)
    with c1:
        fig = px.scatter(filtered, x='Age', y='Value Per M$', color='Performance_Tier',
            color_discrete_map={'Elite': '#ffd700', 'Good': '#00c853', 'Average': '#448aff', 'Developing': '#78909c'},
            hover_data=['Name', 'Overall_Rating', 'Position', 'Team'], opacity=0.7, title='Age vs. Market Value'
        )
        dark_layout(fig)
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        fig = px.box(filtered, x='Position', y='Value Per M$', color='Performance_Tier',
            color_discrete_map={'Elite': '#ffd700', 'Good': '#00c853', 'Average': '#448aff', 'Developing': '#78909c'},
    ...
    with c3:
        ...
        fig = go.Figure(go.Bar(
            x=imp['Importance'], y=imp['Feature'], orientation='h', marker_color='#00c853',
    ...
    c5, c6 = st.columns(2)
    with c5:
        fig = px.histogram(filtered, x='Value Per M$', color='Performance_Tier',
            color_discrete_map={'Elite': '#ffd700', 'Good': '#00c853', 'Average': '#448aff', 'Developing': '#78909c'},
            nbins=50, marginal='rug', title='Market Value Distribution', opacity=0.75)
        dark_layout(fig)
        st.plotly_chart(fig, use_container_width=True)
    with c6:
        pos_tier = filtered.groupby(['Position', 'Performance_Tier']).size().reset_index(name='Count')
        fig = px.bar(pos_tier, x='Position', y='Count', color='Performance_Tier',
            color_discrete_map={'Elite': '#ffd700', 'Good': '#00c853', 'Average': '#448aff', 'Developing': '#78909c'},
            title='Squad Composition by Position', barmode='stack')
        dark_layout(fig)
        st.plotly_chart(fig, use_container_width=True)
...
def page_similar(artifacts):
    ...
    st.markdown("<p style='color: #8899a6; margin: 0; font-size: 1rem;'>Discover comparable talent using multi-dimensional cosine similarity analysis.</p>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    with tab1:
        ...
        selected = st.selectbox("Select a player from the database", player_names,
        ...
        if st.button("Find Comparables", key="sim_existing", type="primary"):
            ...
            player_dict = {
                ...
                'Future Potential': int(row['Future Potential']), 'Total_Stats Score': int(row['Total_Stats Score']),
                'Country': str(row['Country']), 'Team': str(row['Team']), 'Position': str(row['Position'])
            }
            similar = find_similar(player_dict, artifacts, n=7)
            similar = [s for s in similar if s['name'] != selected][:5]

            st.markdown("#### Top 5 Comparables for " + selected)
            ...
            st.dataframe(
                sim_df.style.background_gradient(subset=['Similarity %'], cmap='Greens').format({
                    'Similarity %': '{:.1f}', 'Value (M€)': '€{:.2f}',
                    'OVR': '{:.0f}', 'POT': '{:.0f}', 'Age': '{:.0f}'
                }),
            ...
            fig.add_trace(go.Scatterpolar(
                r=sel_vals + [sel_vals[0]], theta=categories + [categories[0]],
                fill='toself', name=selected, line_color='#00c853', fillcolor='rgba(0,200,83,0.15)'
            ))
            if similar:
                top = similar[0]
                ...
                fig.add_trace(go.Scatterpolar(
                    r=top_vals + [top_vals[0]], theta=categories + [categories[0]],
                    fill='toself', name=top['name'], line_color='#448aff', fillcolor='rgba(68,138,255,0.1)'
                ))
            fig.update_layout(
    ...
    with tab2:
        ...
        if st.button("Find Comparables", key="sim_custom", type="primary"):
            ...
            for s in similar:
                tc = {'Elite': '#ffd700', 'Good': '#00c853', 'Average': '#448aff', 'Developing': '#78909c'}.get(s['tier'], '#78909c')
                render_html(f"""
                <div style="background: #151b2b; border-radius: 12px; padding: 18px; margin-bottom: 12px; border-left: 4px solid {tc};
...
def page_about(artifacts):
    ...
    st.markdown("<p style='color: #8899a6; margin: 0; font-size: 1rem;'>Technical documentation of the ScoutAI Pro machine learning pipeline and evaluation framework.</p>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)

    render_html("""
    <div style="background: #151b2b; border-radius: 16px; padding: 28px; margin-bottom: 28px; border: 1px solid rgba(255,255,255,0.05);">
        <h4 style="color: #00c853; margin-top: 0; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 1.5px;">Preprocessing Pipeline</h4>
        ...
        <p style="color: #8899a6; font-size: 0.85rem; margin-top: 16px; line-height: 1.6;">
            All encoding and scaling operations are fit exclusively on the training partition to prevent data leakage.
            Stratified sampling preserves the imbalanced tier distribution (Elite ~3%, Good ~25%, Average ~40%, Developing ~32%).
        </p>
    </div>
    """, height=200)
    ...
    specs = [
        ("Gradient Boosting", "Tree-based ensemble with 120 estimators, max depth 5. Handles non-linearity, heteroscedasticity, and extreme outliers (Mbappé/Haaland valuations) natively.", "#00c853"),
        ("Support Vector Machine", "RBF kernel SVM with C=10. Captures smooth non-linear manifolds in the Age–Overall–Value space. Margin maximization controls overfitting on the imbalanced Elite tier (~3%).", "#448aff"),
        ("K-Nearest Neighbors", "Instance-based learner with k=10 and distance weighting. Football valuation is inherently comparative; KNN provides interpretable scouting look-alikes.", "#ff6b6b")
    ]
    ...
    perf_data = {
        ...
        'Interpretation': [
            'Variance explained', 'Avg. prediction error', 'Root squared error',
            'Overall correct rate', 'Balanced precision/recall',
            'Unweighted class average', 'Correlation coefficient'
        ]
    }
    ...
    with st.expander("📐 Evaluation Metrics Reference"):
        st.markdown("""
        **Regression Metrics**
        - **R² Score**: Coefficient of determination. Proportion of market value variance explained by the model. Range (-∞, 1]; 1.0 is perfect prediction.
        - **MAE**: Mean Absolute Error in millions of euros. The average magnitude of valuation errors, robust to outliers.
        - **RMSE**: Root Mean Squared Error. Penalizes large misses (e.g., undervaluing a generational talent) more aggressively than MAE.

        **Classification Metrics**
        - **Accuracy**: Overall correct tier prediction rate. Can be misleading with imbalanced classes.
        - **F1-Weighted**: Harmonic mean of precision and recall, weighted by tier support. Balances false positives/negatives across all tiers.
        - **F1-Macro**: Unweighted average of per-tier F1 scores. Treats Elite and Developing tiers as equally important.
        - **MCC**: Matthews Correlation Coefficient. A balanced measure that returns +1 for perfect prediction, 0 for random guessing, and -1 for total disagreement.
        """)
