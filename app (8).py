...
<style>
    ...
    .streamlit-expanderHeader {
        background-color: #151b2b; border-radius: 12px;
        color: #e0e0e0; font-weight: 600;
    }
    .streamlit-expanderContent {
        background-color: #0f1520; border-radius: 0 0 12px 12px;
    }
...
def tier_badge(tier):
    ...
    return f"""
    <span style="background: linear-gradient(90deg, {bg}, {bg}dd); color: {fg};
        padding: 6px 16px; border-radius: 20px; font-weight: 800;
        font-size: 0.85rem; letter-spacing: 1px; display: inline-block;
        box-shadow: 0 2px 10px {bg}40;">{label}</span>
    """
...
def page_home(artifacts):
    ...
    """
    st.markdown(textwrap.dedent(hero), unsafe_allow_html=True)

    m = artifacts['metrics']
    c1, c2, c3, c4 = st.columns(4)
    with c1: card("R² Score", f"{m['r2']:.3f}", "Regression accuracy", "#00c853", "📈")
    with c2: card("MAE", f"€{m['mae']:.2f}M", "Mean absolute error", "#ff6b6b", "📉")
    with c3: card("Dataset", f"{len(artifacts['df']):,}", "Players analyzed", "#448aff", "🗄️")
    with c4: card("Dimensions", f"{len(artifacts['all_feature_names'])}", "Model features", "#ffd700", "🔧")
    ...
    with left:
        ...
        """
        st.markdown(textwrap.dedent(overview), unsafe_allow_html=True)
    ...
    with i1:
        ...
        """
        st.markdown(textwrap.dedent(insight1), unsafe_allow_html=True)
    with i2:
        ...
        """
        st.markdown(textwrap.dedent(insight2), unsafe_allow_html=True)
    with i3:
        ...
        """
        st.markdown(textwrap.dedent(insight3), unsafe_allow_html=True)


def page_valuation(artifacts):
    section_header("💰 Player Valuation Engine",
    ...
    with col_inputs:
        ...
        """
        st.markdown(textwrap.dedent(form_header), unsafe_allow_html=True)
    ...
    with col_results:
        if predict_btn:
            ...
            <div style="background: linear-gradient(145deg, #151b2b, #1a2332); border-radius: 20px;
                padding: 32px; border: 1px solid {tier_color}40;
                box-shadow: 0 8px 32px {tier_color}18; margin-bottom: 24px;">
                ...
                <div style="background: rgba(0,0,0,0.2); border-radius: 12px; padding: 16px; margin-bottom: 20px;">
                    ...
                    <div style="background: #0a0e17; border-radius: 6px; height: 10px; overflow: hidden;">
                        <div style="width: {result['confidence']*100}%; background: linear-gradient(90deg, {tier_color}, {tier_color}80);
            ...
            """
            st.markdown(textwrap.dedent(result_card), unsafe_allow_html=True)
            ...
            for col, (factor, imp) in zip(factor_cols, result['factors']):
                ...
                with col:
                    driver_card = f"""
                    <div style="background: #151b2b; border-radius: 12px; padding: 16px;
                    ...
                    """
                    st.markdown(textwrap.dedent(driver_card), unsafe_allow_html=True)
            ...
            empty_state = """
            <div style="background: #151b2b; border-radius: 20px; padding: 70px 40px;
                text-align: center; border: 2px dashed rgba(0,200,83,0.15); margin-top: 20px;">
            ...
            """
            st.markdown(textwrap.dedent(empty_state), unsafe_allow_html=True)


def page_analytics(artifacts):
    section_header("📊 Player Analytics",
...
def page_similar(artifacts):
    section_header("🔍 Similar Players",
    ...
    with tab2:
        ...
        if st.button("Find Comparables", key="sim_custom", type="primary"):
            ...
            for s in similar:
                ...
                """
                st.markdown(textwrap.dedent(match_card), unsafe_allow_html=True)
...
def page_about(artifacts):
    ...
    """
    st.markdown(textwrap.dedent(pipeline), unsafe_allow_html=True)
    ...
    for col, (title, desc, color) in zip([c1, c2, c3], specs):
        with col:
            ...
            <div style="background: #151b2b; border-radius: 16px; padding: 24px; height: 100%;
                border-top: 4px solid {color}; border: 1px solid rgba(255,255,255,0.05);
            ...
            """
            st.markdown(textwrap.dedent(algo_card), unsafe_allow_html=True)
    ...
    """
    st.markdown(textwrap.dedent(anti_leak), unsafe_allow_html=True)
